...
from config_data.config import API_KEY, API_HOST, DEBUG
...
def track_api_exception(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            log.error_log(exc, 'Ошибка при работе с requests',
...
def get_destinations(destination: str, language: str = 'en_US') -> Dict:
    """
    Получает варианты городов (пунктов назначения) найденные по запросу
    пользователя в виде словаря
    """
...
def hotels_by_destination(
        ...
        sort_order: str = 'PRICE', price_range: Optional[Tuple[int]] = None,
        currency: str = 'USD') -> Dict:
    """Получаем отели по id города (пункта назначения) в виде словаря"""

    if DEBUG:
        print("Enter hotels_by_destination()")

    querystring = {
        "destinationId": destination_id, "pageNumber": "1", "pageSize": "25",
        "checkIn": check_in,
